...
# Adjust labels lenght for figures production
md_lbl_production <- c(
...
data_tbl_assess <- 
    ...
    mutate(n_lbl = if_else(val_type == "NO ASSESSMENT", 
                           paste0(n, " (", round(n/n_papers*100,1), "%)*"),
                           paste0(n, " (", round(n/n_assessment_yes*100,1), "%)†")))
...
data_tbl_rct <- 
    ...
    mutate(n_papers_rct = sum(n),
           n_lbl = paste0(n, " (", round(n/n_papers_rct*100,1), "%)‡"))
...
data_tbl_pilotrct <- 
    ...
    mutate(n_papers_pilot = sum(n),
           n_lbl = paste0(n, " (", round(n/n_papers_pilot*100,1), "%)‡"))
...
data_tbl_assesstype <- 
  ...
  add_row(.after = 1, 
          ...
          n_lbl=paste0(n_assessment_yes, " (", round(n_assessment_yes/n_papers*100,1), "%)*"), 
          n_lbl.rct=paste0(n_papers_rct, " (", round(n_papers_rct/n_assessment_yes*100,1), "%)†"),
          n_lbl.pilot=paste0(n_papers_pilot, " (", round(n_papers_pilot/n_assessment_yes*100,1), "%)†"))
...
data_tbl_assesstype %>%
    ...
    footnote(symbol = 
             c("Percentage based on the total number of studies",
               "Percentage based on the number of studies with an assessment",
               "Percentage based on number of (Pilot) RCT studies"))
...
# FIXME: an estimate of published papers to this matter for 2018. Use real figures instead
prediction_2018 <- 52
...
- The grey bar in the top figure denotes the estimated number of papers in 2018 (three months covered).
...
- The embedded pie chart shows the proportional distribution of types of assessment over all mental disorders.
